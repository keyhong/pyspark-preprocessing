...
import findspark
findspark.init("/opt/mapr/spark/spark-2.4.4")
...
Pandas_Series = TypeVar(pd.Series)
Spark_DataFrame = TypeVar(pyspark.sql.DataFrame)

def transform_coord_format(df: Pandas_DataFrame, case: int=0):
...
class SparkClass(QueryMixin):
    """ spark 이용하여 DB 연결하는 클래스"""

    def __str__(cls):
        return f'Successfully create a {cls.__name__} instance'

    # spark session 속성
    def __init__(self) : 
        self.spark_session = SparkSession.builder \
                                .appName("Spark Session for SOSS RANGLING") \
    ...
    def create_spark_session(self) : 
        spark_session = self.spark_session.getOrCreate()
        
        # warning msg x -> error 만  
...
class QueryMixin:
    ...
    def get_cnt_cctv_info(self) -> pyspark.sql.DataFrame:
        ...
        # 스파크 데이터프레임으로 재변환
        cctv_cnt_df = self.spark_session.createDataFrame(cctv_cnt_df)
...
if __name__ == '__main__':
    ...
    # 추가할 데이터의 파티션 년월 리스트
    appendable_yearMonths = spark_session.find_lastest_yearMonth()

    if not appendable_yearMonths:
        exit()

    # 그리드별 CCTV개수 정보 데이터
    cctv_cnt_df = spark_session.get_cctv_location()
    ...
    for yearMonth in appendable_yearMonths:
        ...
        for time in times:
            ...
            mtr_df = mtr_df.withColumn(
                'mtr_no', F.concat(
                    ( (mtr_df['cnt_x_crd'] - PCELL_X) / 50 ).cast(IntegerType()),
                    F.lit(","), 
                    ( (PCELL_Y - mtr_df['cnt_y_crd']) / 50 ).cast(IntegerType())
                )
            )

            # 기존 범위를 넘어가는 행렬번호는 제거하기
            mtr_df = mtr_df.where( (F.split(str=F.col("mtr_no"), pattern=',')[0] >= 0) & (F.split(str=F.col("mtr_no"), pattern=',')[1] <= 910) )

            # mtr_df에 pcel_stdr_info_df 데이터 결합하기
            mtr_df = mtr_df.join(pcel_stdr_info_df, how='left', on='mtr_no')

            # stdr_tm(기준시간), gu_cd(구코드), pt_stdr_ym(파티션기준년월) 컬럼 생성하기
            mtr_df = mtr_df.withColumn('stdr_tm', F.lit(time).cast(StringType()))
            mtr_df = mtr_df.withColumn('gu_cd', F.col('admd_cd').substr(0, 5).cast(StringType()))
            ...
            # pcel_stdr_info_df의 마지막 그리드 id
            last_pcel_id = spark_session.get_last_pcel_id()
            ...
            unique_add_grid = spark_session.spark_session.createDataFrame(unique_add_grid)

            # 그리드에 cctv 데이터를 결합하고, null인 데이터는 0으로 채우기
            unique_add_grid = unique_add_grid.join(cctv_cnt_df, how='left', on='mtr_no').fillna(0)
