...
def main():
    """pysaprk data preprocessing main function"""
    ...
    for month in [str(i).zfill(2) for i in range(7, 13)]:
        logging.info("%s Start --", month)

        for gu_nm, gu_cd in gu_dict.items():
            logging.info("%s Start --", gu_nm)

            # HDFS에서 인천 공간격자 유동인구 테이블 로딩
            incheon_service_pcell_time_pop = SparkClass.spark.read.csv(
                f"hdfs://{HDFS_HOST}/shared/upload/FloatingPopulation/dsTmznPrnm/incheon_service_pcell_time_pop_2022{month}.csv",
                sep="|",
            ...
            incheon_service_pcell_time_pop = incheon_service_pcell_time_pop.join(
                ic_pcell_df, how="left", on=["x_coord", "y_coord"]
            )

            # Disk I/O를 줄이기 재사용 중간 변수 caching
            incheon_service_pcell_time_pop = incheon_service_pcell_time_pop.filter(
            ...
            incheon_service_pcell_time_pop4 = incheon_service_pcell_time_pop1.join(
                incheon_service_pcell_time_pop2, how="left", on=["std_ymd", "tm", "hcode", "x_coord", "y_coord"]
            )
            incheon_service_pcell_time_pop5 = incheon_service_pcell_time_pop4.join(
                incheon_service_pcell_time_pop3, how="left", on=["std_ymd", "tm", "hcode", "x_coord", "y_coord"]
            )
            incheon_service_pcell_time_pop6 = incheon_service_pcell_time_pop5.filter(
                "h_pop != 0 or w_pop != 0 or v_pop != 0"
            )

            # 성별 연령별 유동인구 테이블 로딩
            incheon_service_pcell_sex_age_pop = SparkClass.spark.read.csv(
                f"hdfs://{HDFS_HOST}/shared/upload/FloatingPopulation/dsAgrdePrnm/incheon_service_pcell_sex_age_pop_2022{month}.csv",
                sep="|",
            ...
            # 구별 필터링
            incheon_service_pcell_sex_age_pop = incheon_service_pcell_sex_age_pop.filter(
                F.col("hcode").like(f"{gu_cd}%")
            )
            ...
            for i in ("h", "w", "v"):
                ...
                for j in ("10", "20", "30", "40", "50", "60", "70"):
                    ...
                    incheon_service_pcell_sex_age_pop = incheon_service_pcell_sex_age_pop.withColumn(
                        f"{i}_w_{j}_r",
                        F.when(F.col(f"{i}_total") == 0, 0).otherwise(F.col(f"{i}_w_{j}") / F.col(f"{i}_total")),
                    )
            ...
            # 와 성별연령별여성인구합계
            merge_pop1 = incheon_service_pcell_time_pop6.join(
                incheon_service_pcell_sex_age_pop, how="left", on=["std_ymd", "hcode", "x_coord", "y_coord"]
            )
            ...
            merge_pop4 = merge_pop3.withColumn(
                ...
                col=F.col("rsd_ppl_fml_10_co")
                + F.col("rsd_ppl_fml_20_co")
                + F.col("rsd_ppl_fml_30_co")
                + F.col("rsd_ppl_fml_40_co")
                + F.col("rsd_ppl_fml_50_co")
                + F.col("rsd_ppl_fml_60_co")
                + F.col("rsd_ppl_fml_70_co")
                + F.col("wrc_ppl_fml_10_co")
                + F.col("wrc_ppl_fml_20_co")
                + F.col("wrc_ppl_fml_30_co")
                + F.col("wrc_ppl_fml_40_co")
                + F.col("wrc_ppl_fml_50_co")
                + F.col("wrc_ppl_fml_60_co")
                + F.col("wrc_ppl_fml_70_co")
                + F.col("vst_ppl_fml_10_co")
                + F.col("vst_ppl_fml_20_co")
                + F.col("vst_ppl_fml_30_co")
                + F.col("vst_ppl_fml_40_co")
                + F.col("vst_ppl_fml_50_co")
                + F.col("vst_ppl_fml_60_co")
                + F.col("vst_ppl_fml_70_co"),
            )
            ...
            merge_pop9.write.format("jdbc").option(
                "url", f"jdbc:postgresql://{POSTGRES_HOST}/{POSTGRES_DATABASE}"
            ).option("driver", "org.postgresql.Driver").option("user", POSTGRES_USER).option(
                "password", POSTGRES_PASSWORD
            ).option("dbtable", "SOSS.DW_PCEL_TMZN_FML_POP_SUM").save(mode="append")

            logging.info("%s End --", gu_nm)

        logging.info("%s End --", month)

    end = time.time()

    # use-maxsplit-arg / C0207 : https://pylint.readthedocs.io/en/latest/user_guide/messages/convention/use-maxsplit-arg.html
    elapsed_time = str(timedelta(seconds=end - start)).split(".", maxsplit=1)[0]
    logging.info("프로그램 소요시간 : %s --", elapsed_time)
